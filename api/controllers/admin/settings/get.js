const moment = require('moment');
const { isNullOrUndefined } = require('util');

module.exports = {


  friendlyName: 'Get all',


  description: 'Get all settings',


  inputs: {
    sorting: {
      type: 'ref'
    }
  },


  exits: {

  },


  fn: async function (inputs, exits) {
    sails.log.debug('calling admin/settings/get-all\ntime: ', moment())
    let sorting = JSON.parse(inputs.sorting)
    let where = {
      deletedAt : null
    }
    let query = {}
    if (!_.isUndefined(sorting.filter)) {
      sorting.filter = JSON.parse(sorting.filter)
      let objectKeys = Object.keys(sorting.filter)
      for (let i = 0; i < objectKeys.length; i++) {
        if (objectKeys[i] == 'q') { continue }
        if (objectKeys[i] == 'id') {
          where.id = sorting.filter[objectKeys[i]]
        }
        else if (sorting.filter[objectKeys[i]] === 'all') { continue }
        else if(objectKeys[i] == 'search')
        {
          where.setting_type =  {contains :  sorting.filter[objectKeys[i]] }
        }
        else {
          where[objectKeys[i]] = sorting.filter[objectKeys[i]]
        }
      }
      if (!_.isUndefined(sorting.filter.q)) {
        where.or = [
          // { first_name: { contains: sorting.filter.q } },
          // { last_name: { contains: sorting.filter.q } },
          { setting_type: { contains: sorting.filter.q } },
          { value: { contains: sorting.filter.q } }
        ]
      }
      query.where = where;
    }
    if (!_.isUndefined(sorting.range)) {
      sorting.range = JSON.parse(sorting.range)
      query.skip = sorting.range[0]
      query.limit = sorting.range[1] + 1
    }
    // if (!_.isUndefined(sorting.sort)) {
    //   sorting.sort = JSON.parse(sorting.sort)
    //   query.sort = `${sorting.sort[0]} ${sorting.sort[1]}`
    // }
    query.sort = `createdAt DESC`
    // query
    // {
    //   where: { email: { contains: sorting.filter.email } },
    //   sort: `${sorting.sort[0]} ${sorting.sort[1]}`,
    //   skip: sorting.range[0],
    //   limit: sorting.range[1] + 1,
    // }
    try {
      sails.log({where} , "==>where")
      sails.log(query.where , "query")

      let settings = await Setting.find(query)
      
    
      const totalSettings = await Setting.count(where)
      sails.log.debug('user/settings/get-all executed\ntime: ', moment())
      return exits.success({ status: true, data: settings, total: `${settings.length}/${totalSettings}` })
    } catch (error) {
      sails.log.error('error at user/settings/get-all error: ', error, '\ntime: ', moment())
      return exits.success({ status: false, data: {}, message: 'Unknown server error' })
    }

  }


};
