...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    sails.log.debug("calling invitee/list\ntime: ", moment());
    ...
    if (!_.isUndefined(sorting.filter)) {
      ...
      if (!_.isUndefined(sorting.filter.search)) {
        where.or = [
          // { first_name: { contains: sorting.filter.q } },
          // { last_name: { contains: sorting.filter.q } },
          { full_name: { contains: sorting.filter.search } },
          { email: { contains: sorting.filter.search } },
        ];
      }
    }
    query.where = where;
    ...
    try {
      ...
      sails.log({ where }, "==>where");
      sails.log(query.where, "qeury");
      sails.log.debug("invitee/list executed\ntime: ", moment());
