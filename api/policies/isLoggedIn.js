...
module.exports = async (req, res, next) => {
	...
	try {
		let extra_attributes = await User.findOne({ where: { user_id: user.id }, select: ["first_name", "last_name", "is_active", "is_blocked", "is_form_submitted", "currentUser","available_amount","account_id"] })
		...
		if (!extra_attributes) {
			...
			extra_attributes = await User.findOne({ where: { user_id: user.id }, select: ["first_name", "last_name", "is_active", "is_blocked" , "currentUser"] })
		}
		if (extra_attributes) {
